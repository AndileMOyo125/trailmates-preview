    ...
    <nav className="fixed w-full z-50 bg-white/80 backdrop-blur-md border-b border-stone-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20">
          <div className="flex-shrink-0 flex items-center space-x-2">
            <span className="text-2xl font-bold tracking-tight text-emerald-800 font-serif">
              TRAIL MATES
            </span>
          </div>
      ...
      >
        <div className="flex flex-col h-full justify-center items-center space-y-8">
          <button
            onClick={toggleMenu}
            className="absolute top-6 right-6 p-2 text-stone-600"
          >
            <X className="w-8 h-8" />
          </button>
          {navLinks.map((link) => (
          ...
          <div className="pt-8">
            <a
              href="https://instagram.com"
              className="flex items-center space-x-2 text-stone-500"
            >
