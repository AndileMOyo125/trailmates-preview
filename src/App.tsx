    ...
    <div className="min-h-screen bg-stone-50 text-stone-900 selection:bg-emerald-100 selection:text-emerald-900 overflow-x-hidden">
      ...
      <section className="relative h-screen flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 text-center text-white px-4 max-w-4xl">
          <ScrollReveal>
            <h1 className="fluid-heading mb-4 leading-tight">
              Trail Mates
            </h1>
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              ...
              >
                Join the Next Walk <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
              </a>
            </div>
          </ScrollReveal>
        </div>
      ...
      <section id="walks" className="py-24 px-4 bg-white">
        <div className="max-w-7xl mx-auto">
          <ScrollReveal className="text-center mb-16">
            <h2 className="text-4xl md:text-5xl font-serif text-stone-900 mb-4">Weekly Walk Days</h2>
            <p className="text-xl text-stone-600">Join us for a midweek reset or a weekend escape</p>
            <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {['Wednesday', 'Friday', 'Saturday', 'Sunday'].map((day: string) => (
                <div key={day} className="bg-stone-50 border border-stone-200 p-6 rounded-2xl text-center shadow-sm hover:shadow-md transition-shadow">
                  <span className="text-emerald-700 font-bold text-lg">{day}</span>
                </div>
              ))}
            </div>
          </ScrollReveal>
            ...
            <ScrollReveal>
              <div className="space-y-8">
                {FEATURES.map((feature: HikeFeature, idx: number) => (
                  <div key={idx} className="flex items-start gap-4 p-6 bg-stone-50 rounded-2xl hover:translate-x-2 transition-transform border border-transparent hover:border-emerald-100">
                    <div className="text-emerald-600 bg-emerald-50 p-3 rounded-xl">
                      {feature.icon}
                    </div>
                    <div>
                      <p className="text-lg font-medium text-stone-800">{feature.text}</p>
                    </div>
                  </div>
                ))}
              </div>
      ...
      <section id="hikes" className="py-24 px-4 bg-emerald-900 text-white overflow-hidden relative">
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          <ScrollReveal className="text-center mb-16">
            <h2 className="text-4xl md:text-5xl font-serif mb-6">Trail Mates Hikes</h2>
            <div className="h-1 w-24 bg-emerald-500 mx-auto mb-8"></div>
            <p className="text-xl text-emerald-100 max-w-2xl mx-auto">
              Midweek Reset & Weekend Escape. Whether you're walking for fitness, fresh air, or connection — Trail Mates is about showing up and enjoying the journey.
            </p>
          </ScrollReveal>
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
             <ScrollReveal>
               <div className="bg-white text-stone-900 p-8 md:p-12 rounded-[2.5rem] shadow-2xl relative overflow-hidden group hover:-translate-y-2 transition-transform duration-500">
                  ...
                  <h3 className="text-3xl font-serif mb-2">Wednesdays & Saturdays</h3>
                  <div className="flex items-baseline gap-2 mb-8">
                    <span className="text-5xl font-bold text-emerald-800">R450</span>
                  ...
                  <div className="mb-8">
                    <h4 className="text-sm font-bold uppercase tracking-widest text-stone-400 mb-6 flex items-center gap-2">
                      <span className="w-6 h-px bg-stone-200"></span> What's Included
                    </h4>
                  ...
                  <div className="pt-8 border-t border-stone-100">
                    <h4 className="text-sm font-bold uppercase tracking-widest text-stone-400 mb-4">Please Note:</h4>
             ...
             <ScrollReveal>
               <div className="grid grid-cols-2 gap-4">
                  <img src="https://images.unsplash.com/photo-1501555088652-021faa106b9b?q=80&w=2073&auto=format&fit=crop" className="rounded-2xl shadow-lg aspect-[3/4] object-cover" alt="Hiking path" />
                  <img src="https://images.unsplash.com/photo-1464822759023-fed622ff2c3b?q=80&w=2070&auto=format&fit=crop" className="rounded-2xl shadow-lg aspect-[3/4] object-cover translate-y-8" alt="Mountain view" />
               </div>
             </ScrollReveal>
          </div>
        </div>
      ...
      <section id="contact" className="py-24 px-4 bg-stone-50 text-center">
        <ScrollReveal>
          <div className="max-w-4xl mx-auto bg-white p-12 rounded-[3rem] shadow-xl border border-stone-100">
            <h2 className="text-4xl md:text-5xl font-serif text-stone-900 mb-6">Ready to Join Us?</h2>
            <p className="text-xl text-stone-600 mb-10">
              Join the WhatsApp group for walk updates, announcements, and location details!
            </p>
            <div className="flex flex-col sm:flex-row gap-6 justify-center items-center">
              ...
              >
                <MessageCircle className="w-6 h-6" /> WhatsApp: {WHATSAPP_NUMBER}
              </a>
            </div>
            <p className="mt-8 text-stone-400 font-medium">@trailmates_trailers on Instagram</p>
          </div>
        </ScrollReveal>
      </section>
      ...
      <footer className="py-12 px-4 bg-stone-900 text-stone-500 border-t border-stone-800">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-8">
          ...
          <div className="flex gap-8 text-sm font-medium">
            ...
            <a href={WHATSAPP_LINK} className="hover:text-white transition-colors">Join Group</a>
          </div>
          <p className="text-sm">© {new Date().getFullYear()} Trail Mates. All rights reserved.</p>
        </div>
      </footer>
    </div>
  );
